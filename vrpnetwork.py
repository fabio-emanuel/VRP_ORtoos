...
class Network:
    ...
    def load_data(self, nodes_file, links_file):
        """Carrega nós e links a partir de arquivos CSV, validando a existência dos nós."""
        ...
        # Adicionar links com verificação de existência dos nós
        for _, row in links_df.iterrows():
            node_a_id = row['a_node']
            node_b_id = row['b_node']
            
            # Verifica se ambos os nós existem no dicionário self.nodes
            if node_a_id in self.nodes and node_b_id in self.nodes:
                link = Link(row['ID'], node_a_id, node_b_id, 
                            row['Vel'], row['Lenght'], row['time_ab'], row['Dir'])
                self.add_link(link)
            else:
                # Opcional: imprimir um aviso sobre links ignorados
                missing = []
                if node_a_id not in self.nodes: missing.append(str(node_a_id))
                if node_b_id not in self.nodes: missing.append(str(node_b_id))
                print(f"Aviso: Link {row['ID']} ignorado. Nó(s) inexistente(s): {', '.join(missing)}")        
    ...
    def export_all_links_to_shapefile(self, output_file):
        """Exporta todos os links da rede para um shapefile."""
        if not self.links:
            print("Nenhum link carregado na rede.")
            return
        ...
        for u, v, data in self.graph.edges(data=True):
            print(u,v)
            # Obtém coordenadas dos nós (lat, lon)
            coord_a = self.graph.nodes[u]['pos']
            coord_b = self.graph.nodes[v]['pos']
            
            # Converte para (lon*1M, lat*1M) conforme padrão do seu código
            # O Point no shapely/geopandas costuma ser (x, y) -> (Long, Lat)
            line_coords = [
                (coord_a[1] * 1000000, coord_a[0] * 1000000),
                (coord_b[1] * 1000000, coord_b[0] * 1000000)
            ]
            
            line = LineString(line_coords)
            
            # Adiciona os atributos do link
            features.append({
                'link_id': data.get('id'),
                'node_a': u,
                'node_b': v,
        ...
        # Exportar
        gdf.to_file(output_file, driver='ESRI Shapefile')
        # Opcional: exportar CSV também como você fez nos outros métodos
        gdf.drop(columns='geometry').to_csv(output_file + '_links.csv', index=False)

        print(f"Total de {len(features)} links exportados para {output_file}")
